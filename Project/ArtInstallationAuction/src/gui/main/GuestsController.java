    ...
    @Override
	  ...
	  public void initialize(URL url, ResourceBundle rb) { //set userName label with the string in login
       index = User_single.User_Info.getInstance().getID();
	     ...
	     c3.setCellValueFactory(new PropertyValueFactory("Surname"));
	     c4.setCellValueFactory(new PropertyValueFactory("email"));
	     //c5.setCellValueFactory(new PropertyValueFactory("Address"));
	     c6.setCellValueFactory(new PropertyValueFactory("Balance"));
     
     try {
         tabel2.getItems().addAll(Main.polka.getActiveUsers());
     } catch (Exception e) {
         e.printStackTrace();
     }
    }
